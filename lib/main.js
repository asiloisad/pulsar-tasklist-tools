...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      atom.commands.add("atom-text-editor:not([mini])", {
        ...
        "tasklist-tools:go-to-next-tick": () => this.goToNextTick(),
  ...
  goToNextTick() {
    ...
    let pass = false;
    editor.scanInBufferRange(regExp, [[curPos.row + 1, 0], [1e9, 1e9]], (iterator) => {
      editor.setCursorBufferPosition([iterator.range.end.row, 1e9], { autoscroll: true });
      iterator.stop();
      pass = true;
    });
    if (!pass) {
      editor.scanInBufferRange(regExp, [[0, 0], [curPos.row + 1, 0]], (iterator) => {
        editor.setCursorBufferPosition([iterator.range.end.row, 1e9], { autoscroll: true });
        iterator.stop();
      });
    }
