...
module.exports = {
  ...
  task(mode) {
    ...
    editor.mutateSelectedText((selection) => {
      ...
      let range = [
        ...
        [
          rbase.end.row,
          selection.isSingleScreenLine() || rbase.end.column > 0 ? 1e9 : 0,
        ],
      ];
  ...
  tickMutate(editor, range, mode) {
    editor.scanInBufferRange(/^([\t ]*)(•|▷|☐|✔|✘)/gm, range, (iterator) => {
      ...
      editor.setTextInBufferRange(
        [
          [iterator.range.end.row, iterator.range.end.column - 1],
          iterator.range.end,
        ],
        symbol
      );
  ...
  moveItemsToBufferRow(editor, selection, destinationRow) {
    editor.transact(() => {
      ...
      selection.setBufferRange([
        ...
        [
          rbase.end.row +
            (selection.isSingleScreenLine() || rbase.end.column > 0),
          0,
        ],
      ]);
      ...
      (iterator) => {
        editor.transact(() => {
          for (let selection of editor.getSelections()) {
            this.moveItemsToBufferRow(
              editor,
              selection,
              iterator.range.start.row + 1
            );
          }
          iterator.stop();
        });
    ...
    editor.backwardsScanInBufferRange(
      ...
      (iterator) => {
        editor.setCursorBufferPosition([
          iterator.range.start.row,
          Point.INFINITY.column,
        ]);
        iterator.stop();
      }
    );
  ...
  activateHandler() {
    atom.workspace
      .getElement()
      .addEventListener("mousedown", this.eventHandlerBinded, {
        capture: true,
        passive: true,
      });
  },
  ...
  deactivateHandler() {
    atom.workspace
      .getElement()
      .removeEventListener("mousedown", this.eventHandlerBinded, {
        capture: true,
        passive: true,
      });
  },
